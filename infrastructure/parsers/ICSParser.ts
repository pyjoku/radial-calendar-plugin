 ...
 * Uses yielding to prevent blocking the main thread with large calendars
 */
export async function parseICS(content: string): Promise<ParsedEvent[]> {
  ...
  for (let i = 0; i < lines.length; i++) {
    // Yield every 200 lines to keep UI responsive
    if (i > 0 && i % 200 === 0) {
