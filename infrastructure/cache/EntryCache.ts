...
export class EntryCache {
  ...
  addEntry(entry: CalendarEntry): void {
    // Remove existing entry for this ID if present (use entry.id, not filePath)
    // This allows multiple entries per file (e.g., birthday + death day)
    this.removeEntry(entry.id);

    // Add to path index by ID (allows multiple entries per file)
    this.entriesByPath.set(entry.id, entry);
  ...
  /**
   * Removes an entry from the cache by ID
   * @param entryId - ID of the entry to remove (can be filePath or filePath#suffix)
   */
  removeEntry(entryId: string): void {
    const existing = this.entriesByPath.get(entryId);
    if (!existing) return;

    // Remove from path index
    this.entriesByPath.delete(entryId);
    ...
    if (existing.isAnniversary) {
      ...
      this.log(`Removed anniversary entry: ${entryId}`);
      return;
    }
    ...
    this.log(`Removed entry: ${entryId}`);
  }
